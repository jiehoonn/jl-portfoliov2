...
export const PROJECT_DETAILS: ProjectDetails = {
  ...
  "foundry": "Foundry is a hackathon‑built, production‑minded AI workflow platform that converts natural‑language intents and CSV data into auditable, human‑approved processes—helping nonprofits cut manual workflow time by ~80%. It couples a modular Python/FastAPI backend (node‑based graph executor and LangGraph multi‑agent orchestration that runs propose→rebut→vote debate rounds) with a Next.js React/TypeScript frontend that consumes Server‑Sent Events to stream live agent reasoning into a DebateViewer and approval flow."
};
...
export const PROJECT_GITHUB_LINKS: Record<string, string> = {
  ...
  'foundry': 'https://github.com/ebrivera/trellis'
};
